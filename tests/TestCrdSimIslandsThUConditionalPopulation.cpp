...
int main() {
  ...
  auto endowment = static_cast<int>(2 * nb_rounds);
  auto target = static_cast<int>(nb_rounds * group_size);
  int delta = 20;
  ...
  try {
    ...
    EGTTools::RL::DataTypes::DataTableCRD results = sim.run_conditional_population_islandsThU(nb_evaluation_games,
                                                                                              nb_populations,
                                                                                              population_size,
                                                                                              group_size,
                                                                                              nb_generations,
    ...
    EGTTools::OutputHandler::writeToCSVFile("testIslandsThUConditionalPopulation.csv", header, results.data);
