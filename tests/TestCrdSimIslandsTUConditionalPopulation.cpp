...
int main() {
  ...
  auto endowment = static_cast<double>(2 * mean_rounds);
  auto target = static_cast<double>(mean_rounds * group_size);
  ...
  try {
    ...
    EGTTools::RL::DataTypes::DataTableCRD results = sim.run_conditional_population_islandsTU(nb_evaluation_games,
                                                                                             nb_populations,
                                                                                             population_size,
                                                                                             group_size,
                                                                                             nb_generations,
    ...
    EGTTools::OutputHandler::writeToCSVFile("testIslandsTUConditionalPopulation.csv", header, results.data);
