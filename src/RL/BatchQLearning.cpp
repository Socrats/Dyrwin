...
#include <Dyrwin/RL/BatchQLearning.h>

using namespace EGTTools::RL;

BatchQLearning::BatchQLearning(size_t nb_states, size_t nb_actions, size_t episode_length, double endowment,
...
void BatchQLearning::reinforceTrajectory() {
...
void BatchQLearning::reinforceTrajectory(size_t episode_length) {
// Now it will calculate the temporal difference errors of the current trajectory
  // And will store them in the TD error buffer (_batch_updates). It will also
  // increase the counts of every visited state (_batch_count).
  // The reinforcement/update of q-values estimation should be
...
bool BatchQLearning::inferPolicy() {
...
/**
       ...
       * the weights.
       */
void BatchQLearning::resetQValues() {
  _q_values.setRandom();
}

std::string BatchQLearning::type() const {
  return "EGTTools::RL::BatchQLearningAgent";
}

// Getters
double BatchQLearning::alpha() const { return _alpha; }

double BatchQLearning::temperature() const { return _temperature; }

// Setters
void BatchQLearning::setAlpha(double alpha) {
  if (alpha <= 0.0 || alpha > 1.0) throw std::invalid_argument("Learning rate parameter must be in (0,1]");
  _alpha = alpha;
}

void BatchQLearning::setTemperature(double temperature) {
