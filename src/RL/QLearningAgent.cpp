...
void QLearningAgent::reinforceTrajectory() {
  ...
  _q_values(_trajectory_states(last),
      ...
      _alpha * (_payoff + _q_values.row(_trajectory_states(last)).maxCoeff() - _q_values(_trajectory_states(last),
                                                                                         _trajectory_actions(last)));
  for (int i = last - 1; i >= 0; i--) {
    _q_values(_trajectory_states(i),
              _trajectory_actions(i)) += _alpha *
        (_payoff + _lambda * _q_values.row(_trajectory_states(i + 1)).maxCoeff() - _q_values(_trajectory_states(i),
