...
CRDSimulator::CRDSimulator(unsigned int population_size) : population_size(population_size) {
    // Default parameters
    group_size = 6;
    nb_games = 1000;
    ...
    for (unsigned int i = 0; i < population_size; i++) {
        _population[i] = CRDPlayer();
        _populationTypesHash.insert(
                std::make_pair(_population[i].strategy, strategy_fq(_population[i].strategy, 0))
        );
    }

    // Copy population into population tmp
    _population_tmp.swap(_population);

    // Initialize fitness vector
    _fitnessVector = std::vector<double>(population_size);

    // Initialize population indexes
    _population_indexes = std::vector<unsigned int>(population_size);
    std::iota(_population_indexes.begin(), _population_indexes.end(), 0);

    // Initialize group structures
    _group = std::vector<CRDPlayer *>(group_size);
    _group_indexes = std::vector<unsigned int>(population_size);
    std::iota(_group_indexes.begin(), _group_indexes.end(), 0);

    // Initialize Game
    _game = new CollectiveRiskDilemma(3, group_size, target_sum, risk, game_rounds, beta);
...
//        std::pair<std::unordered_map<Strategy, StrategyFrequency, StrategyHasher>::iterator, bool> ok;
        for (i = 0; i < population_size; i++) {
            ...
            _population_tmp[i].strategy.copy(_population[_population_indexes[i]].strategy);

            // Mutate player strategy
            ++_population_tmp[i].strategy;
 ...
 */
std::vector<CRDPlayer *> CRDSimulator::_select_randomly(unsigned int size) {
    ...
    for (unsigned int i = 0; i < size; i++) {
        _group[i] = &_population[_population_indexes[i]];
    }
