...
class PDImitation {
 public:
  ...
  [[nodiscard]] unsigned int pop_size() const { return _pop_size; }

  [[nodiscard]] unsigned int nb_coop() const { return _nb_coop; }

  [[nodiscard]] float mu() const { return _mu; }

  [[nodiscard]] float beta() const { return _beta; }

  [[nodiscard]] float coop_freq() const { return _coop_freq; }

  [[nodiscard]] float result_coop_freq() const { return _final_coop_freq; }

  [[nodiscard]] const MatrixXd &payoff_matrix() const { return _payoff_matrix; }
